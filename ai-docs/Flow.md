...
2. Super-Resolution Pipeline Flow:
   ...
   - `frontend/` (React + Vite): Runs on `localhost:5173`. Proxies `/api` and `/static` to backend port 8000 for same-origin security compliance. Uses route-level code splitting via `React.lazy` and `Suspense` wrapped in a semantic `<main id="main-content">` landmark. `HomePage` displays an instant Sentinel-2 HUD placeholder via `HeroMapViewer`, initializing interactive Leaflet tiles asynchronously via `requestIdleCallback` to protect the critical-path LCP. Interacts with `src/server.py` to provide interactive drag-and-drop upload, real-time before/after comparison slider, and 1-click downloads for PNG and TIFF.
